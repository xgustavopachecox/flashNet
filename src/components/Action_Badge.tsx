...
type ActionType = 'CARD MOVIMENTADO' | 'CARD CRIADO' | 'CARD ARQUIVADO' | 'DETALHES ATUALIZADOS';
...
const Action_Badge: React.FC<ActionBadgeProps> = ({ action }) => {
  ...
  return (
    <span className={`px-[1rem] py-[0.4rem] rounded-full text-[0.65rem] font-black tracking-widest border shadow-sm transition-all duration-300 hover:scale-105 select-none ${getBadgeStyles()}`}>
      {action}
    </span>
  );
};
